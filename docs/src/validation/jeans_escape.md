...
2. **2D to 3D Planetary Coupling**: In the 2D Cartesian simulation grid, fluid mass draining from marker porosity evaluates with the out-of-plane out-of-plane spherical integration length $w_{3\text{D}, m} = 2 r_m = 2 \sqrt{(x_m - x_c)^2 + (y_m - y_c)^2}$. The 3D mass vented from marker porosity is $\Delta M_{3\text{D}} = \sum_m \rho_f \Delta\phi_m A_m w_{3\text{D}, m}$, coupling marker transport directly to the 3D spherical atmosphere; this ensures that $P_{\text{atm}} = M_{\text{atm}} g / (4 \pi R_{\text{planet}}^2)$ evaluates in true Pascals.
