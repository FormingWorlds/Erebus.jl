...
The multi-species H-C-N-S solubility, speciation, and organic devolatilization routines operate as a thermodynamic analysis module in `Erebus.jl` (`Erebus.Physics.Volatiles`). In the 2D Stokes-Darcy dynamical solver, fluid flow models the bulk pore fluid (dominated by water).
