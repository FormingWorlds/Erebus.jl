...
function movie_results(input_path)
    ...
    @views @. gmag[2:Ny, 2:Nx, :] = sqrt(
        (0.5 * (gx[2:Ny, 2:Nx, :] + gx[2:Ny, 1:(Nx - 1), :]))^2 +
        (0.5 * (gy[2:Ny, 2:Nx, :] + gy[1:(Ny - 1), 2:Nx, :]))^2,
    )
    ...
    anim_1 = @animate for i in 1:n_steps
        ...
        E = heatmap(
            ...
            xlim=xlim_p,
            ylim=ylim_p,
            clim=(min_log_PHI1mPHI, max_log_PHI1mPHI),
        )
        if invalid(log_EII_)
            log_EII_[2, 2] = 1e-20
        end
    ...
    anim_2 = @animate for i in 1:n_steps
        ...
        O = heatmap(
            ...
            xlim=xlim_p,
            ylim=ylim_p,
            clim=(min_log_ETAPHI, max_log_ETAPHI),
        )
        if invalid(RHO_)
            RHO_[2, 2] = 1e-20
        end
    ...
    return gif(anim_2, input_path*"/HydrologyPlanetesimals_2.mp4")
end
...
function main()
    ...
    return movie_results(input_path)
end
