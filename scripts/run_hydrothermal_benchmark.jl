...
function main()
    config_path = length(ARGS) >= 1 ? ARGS[1] : joinpath(@__DIR__, "..", "configs", "hydrothermal_benchmark.toml")
    println("="^70)
    ...
    println("Grid resolution: $(cfg.grid.Nx) x $(cfg.grid.Ny) nodes (32x32 cells, dx=$(cfg.grid.xsize/(cfg.grid.Nx-1)) m)")
    println("Planetesimal: R_planet=$(cfg.geometry.rplanet) m, R_crust=$(cfg.geometry.rcrust) m")
    ...
    println("Poroelastic compressibility: beta_s=$(cfg.poroelasticity.betasolid) Pa⁻¹, beta_f=$(cfg.poroelasticity.betafluid) Pa⁻¹")
    println("-"^70)
    ...
    files = sort(filter(f -> startswith(f, "output_") && endswith(f, ".jld2"), readdir(output_dir)))

    @printf("%-10s %-12s %-12s %-14s %-14s %-12s\n",
            "Timestep", "Time [Ma]", "T_max [K]", "max(|qD|) [m/s]", "max(k_eff) [m²]", "P_eff_min [MPa]")
    @printf("%-10s %-12s %-12s %-14s %-14s %-12s\n",
            "-"^8, "-"^10, "-"^10, "-"^12, "-"^12, "-"^10)
    ...
    for f in files
        ...
        for i in 1:Ny1, j in 2:Nx1-1
            ...
            if rx_val > 0.0
                ...
                eta_f = compute_fluid_viscosity(T_vx, 1;
        ...
        @printf("%-10d %-12.4f %-12.2f %-14.4e %-14.4e %-12.3f\n",
                step, t_Ma, t_max, max_q, max_k, min_peff_MPa)
    end
    println("="^70)
end
